...
const UserSessionManagementScreen= () => {
  ...
  const fetchSession = async (userEmail) => {
    try {
      const res = await fetch(`${BASE_URL}/api/rideSession/active/${userEmail}`);
      if (!res.ok) {
        setSession(null);
        setLoading(false);
        return;
  ...
  return (
    <View style={styles.container}>
      ...
      {session ? (
        ...
        <Text style={styles.noSessionText}>You are not in any active session.</Text>
      )}
    </View>
  );
};
...
export default UserSessionManagementScreen;
