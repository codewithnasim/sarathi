...
const UserNotificationScreen = () => {
  ...
  const fetchNotifications = async (email) => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/notifications/getnotifications/${email}`
      );
  ...
  const markAsRead = async (notificationId) => {
    try {
      await axios.patch(
        "http://192.168.245.154:5000/api/notifications/markasread",
        { notificationId }
      );
  ...
  const deleteNotification = async (notificationId) => {
    try {
      await axios.delete(
        `http://192.168.245.154:5000/api/notifications/delete/${notificationId}`
      );
      setNotifications((prev) =>
