...
const UserProfileAdmin = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        console.log(`Making request to: https://sarathi-backend-file.onrender.com/api/admin/user/${email}`);
        const response = await axios.get(`https://sarathi-backend-file.onrender.com/api/admin/user/${email}`);
  ...
  const photo = user.photo ? `https://sarathi-backend-file.onrender.com${user.photo}` : 'https://via.placeholder.com/150';
