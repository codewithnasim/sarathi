...
import { 
  View, Text, TouchableOpacity, FlatList, StyleSheet, Image, Alert, ActivityIndicator 
} from "react-native";
import axios from "axios";
import { useNavigation } from "@react-navigation/native";
import { Ionicons } from "@expo/vector-icons";

const BASE_URL = "https://sarathi-backend-file.onrender.com"; // Ensure this is correct
...
const AllDriversScreen = () => {
  ...
  const fetchDrivers = async () => {
    ...
    setError(false);
    try {
      const response = await axios.get(`${BASE_URL}/api/admin/all-drivers`);
    ...
    Alert.alert(
      ...
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              const response = await axios.post(`${BASE_URL}/api/admin/toggle-driver-status`, { driverId });

              if (response.data.success) {
                setDrivers((prevDrivers) =>
                  prevDrivers.map((driver) =>
                    driver._id === driverId ? { ...driver, isBlocked: !isBlocked } : driver
                  )
                );

                Alert.alert("Success", `Driver has been ${action.toLowerCase()}ed.`);
              } else {
                Alert.alert("Error", response.data.message || `Failed to ${action.toLowerCase()} driver.`);
              }
            } catch (error) {
  ...
  const handleRemove = async (driverId) => {
    Alert.alert("Remove Driver", "Are you sure you want to remove this driver?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Remove",
        onPress: async () => {
          try {
            const response = await axios.delete(`${BASE_URL}/api/admin/delete-driver/${driverId}`);

            if (response.data.success) {
              setDrivers((prevDrivers) => prevDrivers.filter((driver) => driver._id !== driverId));
              Alert.alert("Success", "Driver has been removed.");
            } else {
              Alert.alert("Error", response.data.message || "Failed to remove driver.");
            }
          } catch (error) {
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.card}>
              <Image 
                source={{ uri: item.photo ? `${BASE_URL}${item.photo}` : "https://via.placeholder.com/70" }} 
              ...
              <View style={styles.buttonContainer}>
                ...
                <TouchableOpacity 
                  style={[styles.button, item.isBlocked ? styles.unblockButton : styles.blockButton]} 
                  onPress={() => handleBlock(item._id, item.isBlocked)}
                >
                  <Ionicons name={item.isBlocked ? "checkmark-circle-outline" : "ban-outline"} size={22} color="white" />
                ...
                <TouchableOpacity 
                  ...
                  onPress={() => navigation.navigate("SendNotificationUser", { name: item.name, email: item.email })}
                >
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { fontSize: 22, fontWeight: "bold", marginBottom: 10, color: "#fff", textAlign: "center" },
  ...
  driverName: { fontSize: 18, fontWeight: "bold", color: "#333", textAlign: "center" },
  ...
  buttonText: { color: "white", fontSize: 14, marginLeft: 5, fontWeight: "600" },

  errorContainer: { alignItems: "center", marginTop: 20 },
  errorText: { fontSize: 16, color: "red" },
  retryButton: { backgroundColor: "red", padding: 10, borderRadius: 5, marginTop: 10 },
